...
async function review(ctx, page){
  ...
  for(const width of [320,390,768,1440])for(const [i,route] of routes.entries()){
    ...
    for(const href of await root.locator('a[href]').evaluateAll(ns=>ns.map(n=>n.getAttribute('href'))))assert(routes.includes(href)||href.startsWith('#')||href==='/dev-preview/marketplace'||href==='/dev-preview/list-a-product'||/^\/dev-preview\/marketplace\/p09-preview-listing-[0-3]-[0-3]$/.test(href)||href==='/dev-preview/farmers'||href==='/dev-preview/list-your-farm'||/^\/dev-preview\/farmers\/p09-preview-farmer-0[1-4]$/.test(href),'allowlisted href '+href);
