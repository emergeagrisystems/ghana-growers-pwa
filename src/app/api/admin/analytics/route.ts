...
export async function GET(request: Request) {
  ...
  const [
    ...
    readTable("suppliers", "select=id,slug,company_name,category,products_services,region,district,verification_status,status,is_featured,featured_until,featured_note,created_at&limit=1000"),
    readTable("marketplace_listings", "select=id,slug,product_name,category,region,district,seller_name,seller_type,owner_type,owner_id,owner_name,quantity,unit,whatsapp_number,status,availability,price_range,description,internal_operations_notes,image_url,image_urls,is_featured,featured_until,featured_note,created_at&limit=1000"),
