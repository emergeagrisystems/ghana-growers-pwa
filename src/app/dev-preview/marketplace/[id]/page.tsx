...
export default function Page({params,searchParams}:{params:{id:string};searchParams:{from?:string;state?:string}}){if(process.env.VERCEL_ENV==="production")notFound();const items=getMarketplaceFixtures(),item=items.find(i=>i.id===params.id);if(!item)notFound();return <MarketplacePreviewFrame><MarketplacePreviewDetail item={item} back={safeMarketplaceReturn(typeof searchParams.from==="string"?searchParams.from:undefined,items)} scenario={typeof searchParams.state==="string"?searchParams.state:""}/></MarketplacePreviewFrame>;}
