...
export function PageHero({ eyebrow, title, description, children, variant = "default" }: PageHeroProps) {
  ...
  return (
    <section className="border-b border-leaf-900/10 bg-earth-50">
      <div className={`mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 ${isCompact ? "py-10 sm:py-12" : "py-20 sm:py-24"}`}>
        <p className="gg-eyebrow">{eyebrow}</p>
        <h1 className={`${isCompact ? "gg-editorial-heading mt-3 text-3xl leading-tight sm:text-5xl" : "mt-4 gg-hero-title"} sm:max-w-4xl`}>{title}</h1>
