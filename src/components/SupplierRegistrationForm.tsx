...
type FormErrors = Partial<Record<keyof SupplierRegistrationPayload, string>>;
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-md border border-leaf-900/10 bg-white p-5 shadow-soft sm:p-6">
      <div className="grid gap-4 md:grid-cols-2">
        <TextField label="Company Name" name="companyName" value={form.companyName} error={errors.companyName} onChange={(value) => updateField("companyName", value)} />
        ...
        <label className="grid gap-2 text-sm font-bold text-ink/75">
          Supplier Category
          <select className={fieldClass} value={form.supplierCategory} onChange={(event) => updateField("supplierCategory", event.target.value)}>
        ...
        <TextField label="Website" name="website" type="url" value={form.website} error={errors.website} onChange={(value) => updateField("website", value)} placeholder="https://example.com" />
        <TextAreaField label="Products/Services Offered" name="productsServicesOffered" value={form.productsServicesOffered} error={errors.productsServicesOffered} onChange={(value) => updateField("productsServicesOffered", value)} placeholder="Example: seeds, irrigation pumps, cold storage, transport, packaging" />
        <TextAreaField label="Description" name="description" value={form.description} error={errors.description} onChange={(value) => updateField("description", value)} placeholder="Describe your company, service coverage, capacity, and ideal customers." />
      </div>
