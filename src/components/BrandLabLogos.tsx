...
export function ConnectionWordmarkLogo({ compact = false, className, accent = "#E76F51" }: LogoProps) {
  const style = { "--logo-accent": accent } as CSSProperties;

  return (
    <svg className={className} style={style} viewBox={compact ? "0 0 52 52" : "0 0 330 72"} role="img" aria-label="Ghana Growers connected wordmark logo concept">
      {compact ? (
