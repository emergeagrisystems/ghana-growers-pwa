...
export function PublicCardRail({children,label,className}:{children:ReactNode;label:string;className:string}){
 ...
 function sync(){const node=track.current;if(!node)return;const width=node.firstElementChild?.getBoundingClientRect().width??0;const gap=parseFloat(getComputedStyle(node).columnGap)||0;setOverflow(node.scrollWidth>node.clientWidth+1);setPosition(Math.max(0,Math.min(count-1,Math.round(node.scrollLeft/(width+gap)))))}
 useEffect(()=>{const node=track.current;if(!node)return;const observer=new ResizeObserver(sync);observer.observe(node);return()=>observer.disconnect()},[]);
