    ...
    <section className={`${styles.section} ${styles.sage}`} aria-labelledby="homepage-directory">
      <div className={styles.sectionInner}><div className={styles.headingRow}><h2 id="homepage-directory">{copy.directoryTitle}</h2><UnavailableAction label={copy.directoryActions[directoryTab]} /></div>
        ...
        <div id="directory-panel" role="tabpanel" aria-labelledby={`directory-tab-${directoryTab}`} tabIndex={0}>
          {!directoryReady ? <p className={styles.empty}>{copy.directoryUnavailable}</p> : profiles.length === 0 ? <p className={styles.empty}>No Preview fixtures match this category.</p> : <div className={styles.cards}>{profiles.map(entry => <article key={entry.id}><div className={styles.profilePlaceholder}><span aria-hidden="true">{entry.kind === "farmer" ? "PF" : "PS"}</span><p>{entry.kind === "farmer" ? copy.profilePlaceholder : copy.servicePlaceholder}</p></div><h3>{entry.name}</h3><p className={styles.availability}>{copy.fixtureLabel}</p><p>{entry.region}</p><p>{entry.products.join(", ")}</p><p className={styles.availability}>Profile destination unavailable</p></article>)}</div>}
        </div>
      </div>
    </section>
