...
export function AmendedHomepage({data,media,heroCandidates}: {data: HomepageData; media: HomepageMedia; heroCandidates?: readonly HeroCandidate[]}) {
  ...
  return <section className={`${styles.page} ${foundationFont.variable}`} data-amended-homepage aria-label={copy.review}>
    <a className={styles.skipLink} href="#homepage-content">Skip to page content</a>
    ...
    <header id="homepage-content" tabIndex={-1} className={styles.hero} data-homepage-hero>
    ...
    <section className={`${styles.section} ${styles.sage}`} aria-labelledby="homepage-directory">
      <div className={styles.sectionInner}><div className={styles.headingRow}><h2 id="homepage-directory">{copy.directoryTitle}</h2>{directoryTab === 1 ? <a className={styles.action} href="/dev-preview/suppliers-services">{copy.directoryActions[directoryTab]}</a> : <UnavailableAction label={copy.directoryActions[directoryTab]} />}</div>
        ...
        <div id="directory-panel" role="tabpanel" aria-labelledby={`directory-tab-${directoryTab}`} tabIndex={0}>
          {!directoryReady ? <p className={styles.empty}>{copy.directoryUnavailable}</p> : profiles.length === 0 ? <p className={styles.empty}>No Preview fixtures match this category.</p> : <MobileCardBrowser key={directoryTab} label={directoryTab === 0 ? "farmer" : "supplier"}>{profiles.map(entry => <article key={entry.id}><div className={styles.profilePlaceholder}><span aria-hidden="true">{entry.kind === "farmer" ? "PF" : "PS"}</span><p>{entry.kind === "farmer" ? copy.profilePlaceholder : copy.servicePlaceholder}</p></div><h3>{entry.name}</h3><p className={styles.availability}>{copy.fixtureLabel}</p><p>{entry.region}</p><p>{entry.products.join(", ")}</p>{entry.kind === "farmer" ? <a className={styles.action} href={`/dev-preview/farmers/${entry.id}`} aria-label={`View ${entry.name} profile`}>View profile</a> : <a className={styles.action} href={`/dev-preview/suppliers-services/${entry.id}`} aria-label={`View ${entry.name} profile`}>View profile</a>}</article>)}</MobileCardBrowser>}
        </div>
      </div>
    </section>
